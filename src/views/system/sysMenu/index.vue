...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      this.$store.dispatch('sysMenu/getMenus', this.sysMenu).then(response => {
        this.sysMenuList = response
    ...
    removeDataById(id) {
      ...
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        // 点击确定，远程调用ajax
        return await this.$store.dispatch('sysMenu/removeById', id)
      }).then((response) => {
        this.fetchData()
        this.$message({
          type: 'success',
    ...
    // 添加
    save() {
      this.$store.dispatch('sysMenu/addMenu', this.sysMenu).then(response => {
    ...
    // 更新
    update() {
      this.$store.dispatch('sysMenu/updateMenu', this.sysMenu).then(response => {
        this.$message.success('操作成功')
        this.dialogVisible = false
        this.fetchData()
      })
